...
import src.utils.transforms as pt
import torch
import numpy as np
import functools
...
class RotationTransformer:  
    ...
    def __init__(self, 
            from_rep='axis_angles', #axis_angle
            to_rep='rotation_6d', 
        ...
        if to_rep != 'matrix':
            ...
            forward_funcs.append(funcs[0])
            inverse_funcs.append(funcs[1])
        # Reverse the order of inverse functions to properly invert the sequence of transformations
    ...
    def _apply_funcs(x: Union[np.ndarray, torch.Tensor], funcs: list) -> Union[np.ndarray, torch.Tensor]:
        ...
        x_: torch.Tensor
        for func in funcs: # Apply each function in the sequence to the data
            x_ = func(x_)
        y = x_
        if isinstance(x, np.ndarray):
            y = x_.numpy()
...
def test():
    ...
    # Import Rotation from scipy for validation
